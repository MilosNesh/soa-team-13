...
func (s *FollowService) IsFollowing(followerId, followedId string) (bool, error) {
	log.Printf("=== FollowService.IsFollowing ===")
	log.Printf("Checking if %s follows %s", followerId, followedId)

	result, err := s.FollowRepo.IsFollowing(followerId, followedId)

	log.Printf("Repository returned: %v, error: %v", result, err)
	return result, err
}
